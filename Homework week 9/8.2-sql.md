...
```SQL
-- SELECT Accel, rtrim(Accel, 'sec') FROM evCars;
...
```SQL
--`Efficiency,FastCharge = rtrim(Efficiency, 'Wh/km'), FastCharge,rtrim(FastCharge, 'km/h')
...
```SQL
--SELECT Efficiency,FastCharge, rtrim(Efficiency, 'Wh/km'), FastCharge,rtrim(FastCharge, 'km/h') FROM evCars;
```
